<?php
class ControllerOMFSettings extends Controller {
	private $error = array();
	...
	public function index() {		
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('omf/settings', 'token=' . $this->session->data['token'], 'SSL'),       		
      		'separator' => ' :: '
   		);
		...
		$this->data['action_connection'] = $this->url->link('omf/settings/save', 'token=' . $this->session->data['token'], 'SSL');		
		$this->data['action_sync'] = $this->url->link('omf/settings/sync', 'token=' . $this->session->data['token'], 'SSL');		
		$this->data['action_save'] = $this->url->link('omf/settings/save', 'token=' . $this->session->data['token'], 'SSL');		
		$this->data['action_test'] = $this->url->link('omf/settings/test', 'token=' . $this->session->data['token'], 'SSL');		
		$this->data['action_flush_products'] = html_entity_decode($this->url->link('omf/settings/flushMobicartProducts', 'token=' . $this->session->data['token'], 'SSL'));		
		$this->data['action_flush_depts'] = html_entity_decode($this->url->link('omf/settings/flushMobicartDepartments', 'token=' . $this->session->data['token'], 'SSL'));		
		$this->data['action_add_depts_and_products'] = html_entity_decode($this->url->link('omf/settings/addDepartmentsAndProducts', 'token=' . $this->session->data['token'], 'SSL'));		
		...
		$this->template = 'omf/settings.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
...
# *******Save settings through merchant/parner api
	public function save() {
		...
		if ($this->validate()) {
			if(is_numeric($result = $this->test())) {				
				$this->load->model('setting/setting');
				$this->request->post['mobicart_store_id'] = $result;
				$this->model_setting_setting->editSetting('omf', $this->request->post);				
				# *******Save settings through merchant/parner api
				$this->session->data['success'] = "Connection established. Settings saved.";
				
			} else {				
				$this->session->data['error'] = $result;
			}				
		} else {
			$this->session->data['error'] = "Invalid data";			
		}
	...
	public function test() {		
		$key = $this->request->post['mobicart_api_key'];
		$mobi = new Mobicart($key);
		$mobi->add_param("user_name", $this->request->post['mobicart_email'], true);		
	...
	private function getOpenCartCatList() {				
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('omf/settings/sync', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'], 'SSL')
			);
 ...
 */
	...
	private function validate() {
    	...
    	if ((utf8_strlen($this->request->post['mobicart_email']) > 96) || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['mobicart_email'])) {
      		$this->error['email'] = $this->language->get('error_email');
    	}

    	if (utf8_strlen($this->request->post['mobicart_api_key']) != 32) {
